...
import { Plus, BookOpen, Clock, BarChart2, Calendar, Trash2, Edit2 } from "lucide-react";
            ...
            <Link href={`/subjects/${subject.id}`}>
              <Card className="h-full bg-card/50 backdrop-blur-sm border-border/50 shadow-lg hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer group overflow-hidden">
                ...
                <CardContent className="p-6 relative">
                  ...
                  <h3 className="font-display font-bold text-xl mb-1 pr-8 text-foreground">{subject.name}</h3>
                  ...
                  <div className="grid grid-cols-2 gap-y-4 gap-x-2 text-sm">
                    <div className="flex items-center gap-2 text-muted-foreground">
                      ...
                      <span className="font-medium text-foreground/80">{subject.completedTopics}/{subject.topicCount} Topics</span>
                    </div>
                    <div className="flex items-center gap-2 text-muted-foreground">
                      <Clock className="w-4 h-4" style={{ color: subject.color }} />
                      <span className="font-medium text-foreground/80">{formatMinutes(subject.totalStudyMinutes)}</span>
                    </div>
                    <div className="flex items-center gap-2 text-muted-foreground">
                      <BarChart2 className="w-4 h-4" style={{ color: subject.color }} />
                      <span className="font-medium text-foreground/80">{subject.performanceScore}% Score</span>
                    </div>
                    <div className="flex items-center gap-2 text-muted-foreground">
                      <Calendar className="w-4 h-4" style={{ color: subject.color }} />
                      <span className="font-medium text-foreground/80">{subject.attendancePercent || 0}% Att.</span>
                    </div>
                  </div>
